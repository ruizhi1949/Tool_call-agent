...
#         self.llm = BaseLLM("models/Qwen3-1.7B")
        tool_info_list = []
    # 打开 JSONL 文件，逐行读取
        with open("prompts/决赛指令集.jsonl", "r", encoding="utf-8") as f:
            for line in f:
                tool = json.loads(line.strip())  # 逐行解析为 JSON 对象
                t_info = (
                    f"函数名: {tool['intent_name']}\n描述: {tool['intent_description']}\n"
                    + f"参数: \n"
                    + "\n".join(
                        [
                            f"  - {list(param.keys())[0]}: {list(param.values())[0]}"
                            for param in tool.get("slots", [])
                        ]
                    )
                )
        ...
        self.system_prompt =
        """
        你是一个工具选择助手，需要根据用户的指令选择合适的工具进行调用。工具的调用格式与python函数一致，为ToolName(param1=value1, param2=value2)。如果需要调用多个工具，工具之前使用'|'连接\n    <例子/>ToolName1(param1=value1, param2=value2)|ToolName2(param1=value1, param2=value2)<例子>\n    对于String类型数值必须用\"\"包裹，参数顺序严格遵守提供的顺序。例如 <tool>Add(num1=1, num2=2)</tool>，<tool>Concat(str1=\"hello\", str2=\"world\")</tool>，\n    如果工具没有参数，则直接写函数名即可，如<tool>GetTime()</tool>。
        """

#         self.system_prompt = f"""
#         你是一个工具选择助手，需要根据用户的指令选择合适的工具进行调用。工具列表如下:\n{tools_info}\n。工具的调用格式与python函数一致，为ToolName(param1=value1, param2=value2)。
#         如果需要调用多个工具，工具之前使用'|'连接\n
#         <例子/>ToolName1(param1=value1, param2=value2)|ToolName2(param1=value1, param2=value2)<例子>\n
#         对于String类型数值必须用\"\"包裹，参数顺序严格遵守提供的顺序。例如 <tool>Add(num1=1, num2=2)</tool>，<tool>Concat(str1=\"hello\", str2=\"world\")</tool>，\n
#         如果工具没有参数，则直接写函数名即可，如<tool>GetTime()</tool>。"。
#        """
...
#         如果用户的指令不需要调用工具，请直接给出回答。如果需要调用工具，必须严格按照上述格式执行。"""
    ...
    def run(self, input_messages) -> str:
        ...
        # 生成响应内容，max_new_tokens 设置为 512
        response_content = self.llm.generate(messages, max_new_tokens=512)

        # 查找工具调用
        tool_calls = re.findall(r"<tool>(.*?)</tool>", response_content, re.DOTALL)

        if tool_calls:
            tool_call = tool_calls[-1].strip()  # 获取最后一个工具调用并去除首尾空白
            response_content = tool_call

            # 检查工具调用是否有括号，如果没有则在末尾添加括号
            if "(" not in tool_call and ")" not in tool_call:
                response_content = tool_call + "()"
        else:
            response_content = response_content.strip()

            # 检查生成的响应内容是否有括号，如果没有则在末尾添加括号
